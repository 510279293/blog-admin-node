...
router.post('/add', function(req, res, next){
  ...
  let loginSql = `INSERT INTO art_list
         ...
         VALUES(?,?,?,?,?,?,?,?)
         `
  let sqlData = [reqData.title,reqData.desc,reqData.keywords,reqData.tags,reqData.content,reqData.editContent,reqData.isdraft,new Date().getTime()]
  let _resData = JSON.parse(JSON.stringify(resData))
...
router.post('/update', function(req, res, next){
  ...
  let reqData = req.body
  let str = []
  for(key of ['title','art_desc','keywords','tags','content','isdraft','editContent']){
    str.push(`${key}='${reqData[key]}'`)
  }
  let sqlStr = `update art_list set ${str.toString()},create_time='${new Date().getTime()}' where id= '${reqData.id}'`
  let _resData = JSON.parse(JSON.stringify(resData))
